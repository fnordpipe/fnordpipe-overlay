...
if [[ -z ${_GOLANG_VCS_VENDOR} ]]; then

_GOLANG_VCS_VENDOR=1
...
_golang-vcs-vendor_fetch() {
	...
	if [[ -z ${EVCS_OFFLINE} ]]; then
		...
		set -- env GOPATH="${EGO_STORE_DIR}" GO15VENDOREXPERIMENT=1 go get -d -t -u -v -x "${EGO_PN}"
		echo "$@"
		"$@" || :
		"$@" || die
